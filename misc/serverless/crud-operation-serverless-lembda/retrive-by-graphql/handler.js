...
const addName = (firstName) => promisify(callback =>
  dynamoDb.put({
    TableName: process.env.DYNAMODB_TABLE,
    Key: { firstName },
  ...
  mutation: new GraphQLObjectType({
    ...
    fields: {
      ...
      changeHname: {
        ...
        type: GraphQLString,
        resolve: (parent, args) => addName(args.firstName),
      },
    },
  }),
});
...
module.exports.query = ((event, context, callback) => {
  console.log(JSON.stringify(event.pathParameters) + "------Query")
  graphql(schema, '{greeting(firstName: "NASHNASH")}')
    .then(
